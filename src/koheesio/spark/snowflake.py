...
class SyncTableAndDataFrameSchema(SnowflakeStep, SnowflakeTransformation):
    ...
    df: Any = Field(default=..., description="The Spark DataFrame")
...
class SynchronizeDeltaToSnowflakeTask(SnowflakeStep):
    ...
    * Merge - only in streaming mode
    ...
    def _merge_batch_write_fn(self, key_columns, non_key_columns, staging_table):
        ...
        def inner(dataframe: Union["sql.DataFrame", "sql.connect.dataframe.DataFrame"], batchId: int):
    ...
    def _compute_latest_changes_per_pk(
        dataframe: Union["sql.DataFrame", "sql.connect.dataframe.DataFrame"],
    ...
    def load(self, df) -> Union["sql.DataFrame", "sql.connect.dataframe.DataFrame"]:
